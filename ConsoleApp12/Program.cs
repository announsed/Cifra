...
partial class Program
{
    static async Task Main()
    {
        ...
        string url = "https://wttr.in/Nizhny+Novgorod?format=j3";

        string response = await client.GetStringAsync(url);
        Root persona = JsonSerializer.Deserialize<Root>(response);
        var wether = persona.weather;
        Console.WriteLine($" Температьура в цельсиях на данный момент: {wether[0].avgtempC}");
        var a = wether[0].totalSnow_cm;
        Console.WriteLine($" Выпадение снега: {a}");
        var curr = persona.current_condition;
        var tt = curr[0].localObsDateTime;
        Console.WriteLine($" Вывожу како-то время: {tt}");
        var wetherURL = JsonSerializer.Deserialize<WeatherUrl>(response);
        Console.WriteLine(wetherURL.value);
