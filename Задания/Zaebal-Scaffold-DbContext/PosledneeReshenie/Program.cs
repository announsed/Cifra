...
using (var dbContext = new AppDbContext())
{
    ...
    var result = orders.Where(x => x.Status == "Отправлен").Where(x => x.Products.Any(y => y.Name == "Телефон"));

    foreach (var a in result) 
    {
        Console.WriteLine(a.OrderId + " " + a.OrderDate + " " + a.User + " " + a.Products + " " + a.Status);
    }
}
